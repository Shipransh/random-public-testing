...
def process_sheet(df):
    ...
    # Create a mapping from the first set of keys to values
    key_value_map = df.iloc[:, col_key_index_1].map(pd.Series(df.iloc[:, col_value_index_1].values, index=df.iloc[:, col_key_index_1]))

    # Create a boolean column that checks if Value_1 matches Value_2 based on Key
    df['Match'] = df.iloc[:, col_key_index_2].map(key_value_map) == df.iloc[:, col_value_index_2]
